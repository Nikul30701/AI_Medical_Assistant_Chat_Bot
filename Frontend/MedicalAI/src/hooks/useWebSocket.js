...
import { addMessage, setWsStatus, setError, selectWsStatus } from '../store/slices/chatSlice'
...
export default function useWebSocket(documentId) {
  ...
  const connect = useCallback(() => {
    ...
    ws.onerror   = () => dispatch(setError('Connection error'))
    ws.onclose   = ({ code }) => {
      dispatch(setWsStatus('disconnected'))
      wsRef.current = null
      if (code !== 1000 && retries.current < 4) {
        retryTimer.current = setTimeout(connect, Math.min(1000 * 2 ** retries.current, 15000))
        retries.current++
      }
    }
  ...
  return { sendMessage, connectionStatus: wsStatus, reconnect }
}
